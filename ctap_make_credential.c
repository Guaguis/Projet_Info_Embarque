...
void ctap_getassertion(void) {
    ...
    // 1) Lire hashed_app_id et clientDataHash depuis l’UART
    serial_read(hashed_app_id, 20);
    serial_read(client_data_hash, 20);

    // 2) Récupérer (cred_id, sk) pour cet app_id
    ...
    // 3) Demande de consentement utilisateur (LED + bouton)
    if (!wait_user_approval()) {
        serial_write_byte(STATUS_ERR_APPROVAL);
        // On efface la clé de la SRAM comme demandé dans l'API memoire.h
        memset(item.sk, 0, sizeof(item.sk));
        return;
    }

    // 4) Extraire la vraie clé privée 20 octets depuis sk[21]
    //    On suppose qu'on a stocké 0 || priv_key20 dans sk
    memcpy(priv_key20, &item.sk[1], 20);

    // 5) Signer clientDataHash avec ECDSA
    ...
    // 6) Succès : envoyer STATUS_OK + credential_id + signature
    serial_write_byte(STATUS_OK);
    serial_write(item.cred_id, 16);
    serial_write(signature, 40);

    // 7) Nettoyage des secrets en RAM
    memset(priv_key20, 0, sizeof(priv_key20));
    memset(item.sk, 0, sizeof(item.sk));
    memset(client_data_hash, 0, sizeof(client_data_hash));
}
